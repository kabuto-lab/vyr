...
const CanvasGrid = () => {
    ...
    useEffect(() => {
        ...
        for (let row = 0; row < rows; row++) {
            ...
            for (let col = 0; col < cols; col++) {
                ...
                if (owner !== null) {
                    ...
                    if (owner === 0)
                        color = '#EF4444'; // Player 1 - Red
                    else if (owner === 1)
                        color = '#3B82F6'; // Player 2 - Blue
                    else if (owner === 2)
                        color = '#10B981'; // Player 3 - Green
                    else if (owner === 3)
                        color = '#F59E0B'; // Player 4 - Yellow
                    ...
                    if (lifetime < 10) {
                        ...
                        ctx.beginPath();
                        ctx.ellipse(centerX + variationX, centerY + variationY, width, // width
                        height, // height
                        0, // rotation
                        0, // start angle
                        2 * Math.PI // end angle
                    ...
                    ctx.beginPath();
                    ctx.arc(offsetX + col * cellWidth + cellWidth / 2, centerY, 1, 0, 2 * Math.PI);
        ...
        if (gameState.gameState === 'battle') {
            ...
            const visibleTentacles = gameState.tentacles.filter(tentacle => {
                ...
                return ((startX >= -padding && startX <= displayWidth + padding &&
            ...
            const tentaclesToDraw = Math.min(visibleTentacles.length, visualEffectQuality === 'high' ? 100 :
                ...
                if (owner !== null) {
                    ...
                    for (let dRow = -2; dRow <= 2; dRow++) {
                        for (let dCol = -2; dCol <= 2; dCol++) {
                            if (dRow === 0 && dCol === 0)
                                continue; // Skip self
                            if (row + dRow < 0 || row + dRow >= rows)
                                continue;
                            if (col + dCol < 0 || col + dCol >= cols)
                                continue;
                            ...
                            if (otherOwner === owner && Math.random() < connectionStrength) {
                                ...
                                if ((x1 >= 0 && x1 <= displayWidth && y1 >= 0 && y1 <= displayHeight) ||
        ...
        if (gameState.gameState === 'battle') {
            ...
            if (visualEffectQuality === 'high')
                maxEffectsToProcess = Math.min(15, 50);
            else if (visualEffectQuality === 'medium')
                maxEffectsToProcess = Math.min(15, 30);
            else
                maxEffectsToProcess = Math.min(15, 10);
            ...
            for (let i = 0; i < visualEffects.length && visibleEffects.length < maxEffectsToProcess; i++) {
                ...
                if (progress < 1) {
                    ...
                    if (effect.type === 'attack' && effect.from && effect.to) {
                        ...
                        isVisible = ((startX >= 0 && startX <= displayWidth && startY >= 0 && startY <= displayHeight) ||
                        ...
                        isVisible = ((pathStartX >= 0 && pathStartX <= displayWidth && pathStartY >= 0 && pathStartY <= displayHeight) ||
            ...
            for (let i = 0; i < visibleEffects.length; i++) {
                ...
                if (effect.type === 'attack' && effect.from && effect.to) {
                    ...
                    /**
                     * Обработка параметрических эффектов
                    ...
                    if (visualEffectQuality === 'high' || effect.type === 'wave') {
                        ...
                        switch (effect.type) {
                            /**
                             * Волновой эффект для захвата и атаки клеток
                             * Wave effect for cell capture and attacks
                             *
                             * Этот эффект создает радиальную волну с 3-кратно увеличенным радиусом,
                             * замедленной анимацией (в 3 раза медленнее) и высоким контрастом цвета.
                             *
                            ...
                            case 'wave': {
                                ...
                                if (rgbColor) {
                                    // Создаем градиент с высоким контрастом (полная непрозрачность в центре, полная прозрачность на краях)
                                    ...
                                    waveGradient.addColorStop(0, `rgba(${rgbColor.r}, ${rgbColor.g}, ${rgbColor.b}, 1.0)`); // Полная непрозрачность в центре
                                    waveGradient.addColorStop(1, `rgba(${rgbColor.r}, ${rgbColor.g}, ${rgbColor.b}, 0.0)`); // Полная прозрачность на краях
                            ...
                            /**
                             * Эффект агрессии - визуализирует нападающие действия вируса
                             * Aggression effect - visualizes attacking actions of the virus
                             *
                             * Создает радиальный градиент с красным цветом, показывающий силу атаки
                            ...
                            /**
                             * Эффект защиты - визуализирует защитные действия вируса
                             * Defense effect - visualizes defensive actions of the virus
                             *
                             * Создает пульсирующий круг, показывающий уровень защиты
                            ...
                            /**
                             * Эффект скорости - визуализирует быстродействие вируса
                             * Speed effect - visualizes the speed of the virus
                             *
                             * Создает желтый круг, показывающий скорость распространения
                            ...
                            /**
                             * Эффект скрытности - визуализирует скрытные действия вируса
                             * Stealth effect - visualizes stealthy actions of the virus
                             *
                             * Создает фиолетовый полупрозрачный круг, показывающий скрытность
                            ...
                            /**
                             * Эффект устойчивости - визуализирует сопротивление вируса
                             * Resistance effect - visualizes the virus's resistance
                             *
                             * Создает зеленую окружность, показывающую уровень устойчивости
                            ...
                            /**
                             * Эффект вирулентности - визуализирует токсичность вируса
                             * Virulence effect - visualizes the toxicity of the virus
                             *
                             * Создает радиальный градиент с красно-розовым цветом, показывающий токсичность
                            ...
                            /**
                             * Эффект победы - визуализирует торжественные события
                             * Victory effect - visualizes celebratory events
                             *
                             * Создает расширяющуюся окружность с частицами, показывающую победу
                            ...
                            /**
                             * Эффект мутации - визуализирует изменчивость вируса
                             * Mutation effect - visualizes the variability of the virus
                             *
                             * Создает радиальный градиент с зеленым цветом и вспышки, показывающие мутации
                            ...
                            /**
                             * Эффект адаптивности - визуализирует способность вируса адаптироваться
                             * Adaptability effect - visualizes the virus's ability to adapt
                             *
                             * Создает фиолетовый градиент с квадратными элементами, показывающие адаптацию
                            ...
                            /**
                             * Эффект выносливости - визуализирует устойчивость вируса к внешним воздействиям
                             * Endurance effect - visualizes the virus's resistance to external factors
                             *
                             * Создает голубой градиент с дополнительной окружностью, показывающий выносливость
                            ...
                            /**
                             * Эффект мобильности - визуализирует подвижность вируса
                             * Mobility effect - visualizes the mobility of the virus
                             *
                             * Создает желтый градиент с лучами, показывающие направления движения
                            ...
                            /**
                             * Эффект интеллекта - визуализирует умственные способности вируса
                             * Intelligence effect - visualizes the mental capabilities of the virus
                             *
                             * Создает фиолетовый градиент с лучами, имитирующими нейронную сеть
                            ...
                            /**
                             * Эффект устойчивости - визуализирует восстановительные способности вируса
                             * Resilience effect - visualizes the recovery abilities of the virus
                             *
                             * Создает зеленую окружность с внутренней окружностью, показывающую устойчивость
                            ...
                            /**
                             * Эффект заразности - визуализирует способность вируса заражать клетки
                             * Infectivity effect - visualizes the virus's ability to infect cells
                             *
                             * Создает синий градиент с частицами, показывающие распространение заразы
                            ...
                            /**
                             * Эффект летальности - визуализирует смертоносность вируса
                             * Lethality effect - visualizes the deadliness of the virus
                             *
                             * Создает красный градиент с лучами, показывающие смертельное воздействие
                            ...
                            /**
                             * Эффект стабильности - визуализирует устойчивость вируса к изменениям
                             * Stability effect - visualizes the virus's resistance to changes
                             *
                             * Создает светло-синий градиент с крестом, показывающий стабильность
                            ...
                            /**
                             * Эффект атаки - визуализирует атакующие действия вируса
                             * Attack effect - visualizes the attacking actions of the virus
                             *
                             * Создает радиальный градиент с цветом игрока, показывающий атаку
                            ...
                            /**
                             * Эффект захвата - визуализирует захват клетки другим вирусом
                             * Capture effect - visualizes a cell being captured by another virus
                             *
                             * Создает радиальный градиент с цветом захватывающего игрока, показывающий захват
                            ...
                            /**
                             * Стандартный эффект - используется для всех остальных типов эффектов
                             * Default effect - used for all other effect types
                             *
                             * Создает стандартный круговой эффект с цветом эффекта
    ...
    return (_jsx("div", { className: "w-full h-full", children: _jsx("canvas", { ref: canvasRef, className: "grid-canvas" }) }));
};
