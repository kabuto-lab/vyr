    ...
    useEffect(() => {
        ...
        if (gameState.gameState === 'battle') {
            const visibleTentacles = gameState.tentacles.filter(tentacle => {
                ...
                const padding = 50;
                return ((startX >= -padding && startX <= displayWidth + padding && startY >= -padding && startY <= displayHeight + padding) ||
                    (endX >= -padding && endX <= displayWidth + padding && endY >= -padding && endY <= displayHeight + padding));
            });
            const tentaclesToDraw = Math.min(visibleTentacles.length, visualEffectQuality === 'high' ? 100 : visualEffectQuality === 'medium' ? 50 : 20);
