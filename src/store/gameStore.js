...
export const useGameStore = create((set) => ({
    ...
    actions: {
        ...
        updateGrid: (newGrid, newCellAge) => set((store) => ({
            gameState: {
                ...store.gameState,
                grid: newGrid,
                cellAge: newCellAge
        ...
        updateTurn: (turn) => set((store) => ({
            gameState: { ...store.gameState, turn }
        })),
        addAttackEffect: (from, to, attacker) => set((store) => {
        ...
        addExpansionEffect: (from, to, player) => set((store) => {
            const colors = ['#EF4444', '#3B82F6', '#10B981', '#F59E0B']; // Player colors
        ...
        addExpansionSourceEffect: (position, player) => set((store) => {
            const colors = ['#EF4444', '#3B82F6', '#10B981', '#F59E0B']; // Player colors
            const playerColor = colors[player];
            const startTime = Date.now();
            const duration = 400; // Default duration
        ...
        addExpansionPathEffect: (from, to, player) => set((store) => {
            const colors = ['#EF4444', '#3B82F6', '#10B981', '#F59E0B']; // Player colors
            const playerColor = colors[player];
            const startTime = Date.now();
            const duration = 400; // Default duration
        ...
        addExpansionTargetEffect: (position, player) => set((store) => {
            const colors = ['#EF4444', '#3B82F6', '#10B981', '#F59E0B']; // Player colors
            const playerColor = colors[player];
            const startTime = Date.now();
            const duration = 400; // Default duration
        ...
        addParameterEffect: (position, type, player) => set((store) => {
            const colors = {
                aggression: '#EF4444',
                defense: '#60A5FA',
                speed: '#FBBF24',
        ...
        addInteractionEffect: (position, type, player) => set((store) => {
        ...
        updateTentacles: (tentacles) => set((store) => ({
            gameState: { ...store.gameState, tentacles }
        })),
        updatePlayers: (players) => set((store) => ({
        ...
        testBattle: () => set((store) => {
            ...
            const updatedPlayers = store.gameState.players.map((player, playerId) => {
                ...
                paramNames.forEach((paramName, index) => {
                    newVirus[paramName] = params[index];
                });
