...
const adjacentCellsCache = new Map();
...
function calculateNextGameState(grid, players, turn, _settings, existingTentacles = [], currentCellAge = []) {
                                        ...
                                        if (Math.abs(dr) + Math.abs(dc) >= 2 && Math.abs(dr) + Math.abs(dc) <= 3) {
                                            ...
                                            if (newRow >= 0 && newRow < 35 && newCol >= 0 && newCol < 70) {
                                                ...
                                                if (newGrid[newRow][newCol] === null) {
                                                    ...
                                                    const hasValidPath = pathCells.some(pathCell => newGrid[pathCell.row][pathCell.col] === owner);
                                                    if (hasValidPath) {
                            ...
                            // Handle mobility-based directional growth
                                ...
                                if (emptyCells.length > 0) {
                                    ...
                                    if (Math.random() < expansionChance) {
                                        ...
                                        let targetCell;
                                        if (player.preferredDirection) {
                                            const preferredCells = emptyCells.filter(adj => adj.row === row + player.preferredDirection[0] &&
    ...
    const existingTentacleMap = new Map();
    for (const tentacle of existingTentacles) {
